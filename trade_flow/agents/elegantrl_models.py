...
class DRLAgent:
    ...
    def get_model(self, model_name, model_kwargs):
        ...
        if model_kwargs is not None:
            try:
                model.break_step = int(2e5)  # break training if 'total_step > break_step'
                model.net_dims = (
                    128,
                    64,
                ...
                model.repeat_times = (
                    16  # repeatedly update network using ReplayBuffer to keep critic's loss small
                )
                model.learning_rate = model_kwargs.get("learning_rate", 1e-4)
                model.state_value_tau = (
                    0.1  # the tau of normalize for value and state `std = (1-std)*std + tau*std`
                )
