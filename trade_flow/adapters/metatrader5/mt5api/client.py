...
from trade_flow.adapters.metatrader5.mt5api.stream_manager import StreamManager
from trade_flow.adapters.metatrader5.mt5api.symbol import Symbol, SymbolInfo, process_symbol_details
from trade_flow.adapters.metatrader5.mt5api.errors import (
    TERMINAL_CONNECT_FAIL,
    SERVER_CONNECT_FAIL,
    TerminalError,
    CodeMsgPair,
)
from trade_flow.adapters.metatrader5.mt5api.utils import ClientException, current_fn_name
from trade_flow.adapters.metatrader5.mt5api.MetaTrader5 import MetaTrader5
...
class MT5Client(MetaTrader5):
    ...
    def connect(self, path: str = "", **kwargs):
        ...
        try:
            ...
            if self.logger:
                ...
                self.logger.error(
                    NO_VALID_ID, TERMINAL_CONNECT_FAIL.code(), TERMINAL_CONNECT_FAIL.msg()
                )
    ...
    def set_conn_state(self, conn_state):
        ...
        self.logger.debug("%s conn_state: %s -> %s" % (id(self), _conn_state, self.conn_state))
    ...
    def req_tick_by_tick_data(
        ...
        if self.market_data_type == MarketDataTypeEnum.REALTIME:
            ...
            self.stream_manager.create_streaming_task(
                f"TICK-{str(req_id)}", [symbol.symbol], self.stream_interval, fetch_latest_tick
            )
    ...
    def req_real_time_bars(
        self, req_id: TickerId, symbol: Symbol, bar_size: int, what_to_show: str, use_rth: bool
    ):
        _current_fn_name = current_fn_name()
