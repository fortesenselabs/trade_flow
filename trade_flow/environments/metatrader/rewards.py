...
class MetaTraderRewardScheme(RewardScheme):
    """An abstract base class for reward schemes for a default MetaTrader environment."""
    ...
    def get_reward(self, portfolio) -> float:
        ...
        ----------
        portfolio : `Portfolio`
            The portfolio associated with the `MetaTraderActionScheme`.
...
class SimpleProfit(MetaTraderRewardScheme):
...
class RiskAdjustedReturns(MetaTraderRewardScheme):
...
class PBR(MetaTraderRewardScheme):
...
def get(identifier: str) -> "MetaTraderRewardScheme":
