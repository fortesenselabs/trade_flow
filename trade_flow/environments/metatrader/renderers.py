...
from trade_flow.environments.metatrader.engine.orders import TradeSide
...
class BaseRenderer(Renderer):
    ...
    def save(self) -> None:
        """Saves the rendering of the `TradingEnvironment`."""
        pass
...
class PlotlyTradingChart(BaseRenderer):
    """Trading visualization for TradeFlow using Plotly.
    ...
    def _create_figure(self, performance_keys: dict) -> None:
        ...
        fig.add_trace(
            go.Candlestick(name="Price", xaxis="x1", yaxis="y1", showlegend=False), row=1, col=1
        )
        fig.update_layout(xaxis_rangeslider_visible=False)

        fig.add_trace(
            go.Bar(name="Volume", showlegend=False, marker={"color": "DodgerBlue"}), row=2, col=1
        )
        ...
        fig.add_trace(
            go.Scatter(mode="lines", name="Net Worth", marker={"color": "DarkGreen"}), row=4, col=1
        )
...
class MatplotlibTradingChart(BaseRenderer):
    """Trading visualization for TradeFlow using Matplotlib
    Parameters
    ---------
    display : bool
        True to display the chart on the screen, False for not.
    ...
    def render_env(
        ...
        self.fig.suptitle(
            "Net worth: $" + str(current_net_worth) + " | Profit: " + str(profit_percent) + "%"
        )
