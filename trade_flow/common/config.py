...
from trade_flow.common import Environment
from trade_flow.core.correctness import PyCondition
from trade_flow.core.uuid import UUID4
from trade_flow.model.data import BarType
from trade_flow.model.identifiers import ComponentId
from trade_flow.model.identifiers import Identifier
...
def resolve_config_path(path: str) -> type[FlowConfig]:
    config = resolve_path(path)
    if not issubclass(config, FlowConfig):
        raise TypeError(f"expected a subclass of `FlowConfig`, was `{type(config)}`")
    return config
...
def tokenize_config(obj: FlowConfig) -> str:
    return hashlib.sha256(obj.json()).hexdigest()


class FlowConfig(msgspec.Struct, kw_only=True, frozen=True):
    """
    ...
    def fully_qualified_name(cls) -> str:
        """
        Return the fully qualified name for the `FlowConfig` class.
...
class DatabaseConfig(FlowConfig, frozen=True):
    """
...
class MessageBusConfig(FlowConfig, frozen=True):
    """
...
class InstrumentProviderConfig(FlowConfig, frozen=True):
    """
...
class OrderEmulatorConfig(FlowConfig, frozen=True):
    """
...
class ActorConfig(FlowConfig, kw_only=True, frozen=True):
    """
...
class ImportableActorConfig(FlowConfig, frozen=True):
    """
...
class LoggingConfig(FlowConfig, frozen=True):
    """
...
class ImportableFactoryConfig(FlowConfig, frozen=True):
    """
...
class ImportableConfig(FlowConfig, frozen=True):
    """
