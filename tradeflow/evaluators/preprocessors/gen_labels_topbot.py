...
def generate_labels_topbot2(df, config: dict):
    ...
    if not column_name:
        raise ValueError(
            f"The 'columns' parameter must be a non-empty string. {type(column_name)}"
        )
    ...
    if function not in ["top", "bot"]:
        raise ValueError(
            f"Unknown function name {function}. Only 'top' or 'bot' are possible"
        )
    ...
    names = config.get(
        "names"
    )  # For example, ['top1_025', 'top1_01'] for two tolerances
    if len(names) != len(tolerances):
        raise ValueError(
            f"'topbot2' Label generator: for each tolerance value one name has to be provided."
        )

    labels = []
    for i, tolerance in enumerate(tolerances):
        df, new_labels = add_extremum_features(
            df,
...
def generate_labels_topbot(
    df, column_name: str, top_level_fracs: list, bot_level_fracs: list
):
...
def find_one_extremum(
    ...
    if is_max:
        ...
        tolerance_val = extr_val / (
            1 - tolerance_frac
        )  # extr_val * (1 + tolerance_frac)
