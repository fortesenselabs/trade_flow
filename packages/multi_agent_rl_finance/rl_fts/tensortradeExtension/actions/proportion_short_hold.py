"""
    Notes on entering a Short implementation with proportion:
    1. Scenario
    - If I have $100 cash.
    - If the borrow requirement is set to 150%
    - If the maintenance margine is set to 25%
    - If the action is to enter a short with 50% fund allocation
    2. Implementation
    - We require 150% of the trade to be in the deposit margine. 
        - (50% * $100) = $50 cash becomes the 150% of whats available
        - $50 / 1.5 = $33.33 is our 100% (the amount that we can short)
        - $16.66 is our 50% additional margin requirement
...
class PSH(TensorTradeActionScheme):
    ...
    Notes :
        ...
        If the agent is currently in a short position and attemps to enter a new short position. The agent must
            first exit the old position and then enter the new position. (we do not allow multiple shorts)
    """
    ...
    def __init__(
        ...
        # USD-TTC
        traing_pair = TradingPair(
            self.cash.balance.instrument, self.broker_asset.balance.instrument
        )
        # sine-wave-USD-TTC
    ...
    def enterShort(self, proportion):
        ...
        borrow_size = total_required_amount.as_float() / self.borrow_requirement
        ...
        # When the agent enters the short position, the total required amount must be transfered (cash -> deposit_margin)
        self.transfer(
            source=self.cash,
        ...
        short_enter_order = proportion_order(
            self.portfolio, source=self.asset, target=self.cash, proportion=1
        )
    ...
    def exitShort(self):
        ...
        if current_asset_quantity > self.borrow_asset:
            ...
            borrow_remainder_cash = (self.borrow_asset - current_asset_quantity).convert(
                self.exchange_pair
            )
    ...
    def transfer(
        self, source: Wallet, target: Wallet, quantity: Quantity, commission: Quantity, reason: str
    ):
        """
