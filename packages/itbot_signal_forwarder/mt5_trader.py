...
from packages.itbot_signal_forwarder.risk_manager import RiskManager
...
class MT5Trader:
    ...
    async def execute_trade(
        self, signal_data: Dict, strategy_name: str = "fixed_percentage", **kwargs
    ) -> None:
        """
        Execute a trade order based on the provided signal data and selected risk management strategy.

        Args:
            signal_data (Dict): Signal data containing trade type, symbol, and strategy details.
        ...
        try:
            ...
            if not trade_type or not symbol:
                raise ValueError(f"Invalid signal data: {signal_data}")

            # Get symbol information and ensure visibility
            symbol_info = self.mt5.symbol_info(symbol)
            if not symbol_info:
                raise ValueError(f"{symbol} not found, cannot perform trade operation")
            if not symbol_info.visible:
                self.logger.warning(f"{symbol} is not visible, attempting to enable...")
                if not self.mt5.symbol_select(symbol, True):
                    raise ValueError(f"Failed to enable symbol: {symbol}")

            # Prepare trade request
            request = self._prepare_trade_request(symbol, trade_type, **kwargs)
            self.logger.info(
