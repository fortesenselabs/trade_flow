    ...
    async def decode(self, msg: tuple):
        ...
        match fn_name.lower():
            ...
            case "req_market_data_type":
                task = self.create_task(self.handler.process_market_data_type, req_id=req_id, market_data_type=MarketDataTypeEnum(response))
            case "req_symbol_details" | "req_matching_symbols": # | 
                task = self.create_task(self.handler.process_symbol_details, req_id=req_id, symbol_infos=response)
            case "req_tick_by_tick_data":
                task = self.create_task(self.handler.process_tick_by_tick_bid_ask,                     
                                        req_id=req_id, 
            ...
            case "req_real_time_bars":
                bar = response[0]
                task = self.create_task(self.handler.process_realtime_bar,                     
                                        req_id=req_id, 
                                        time=bar.time,
                                        open_=bar.open_,
                                        high=bar.high,
