...
import numpy as np
from .depth_processing import depth_to_df
...
def get_symbol_files(symbol, path=in_path_name):
    """
    Get a list of file paths containing data for the given symbol.
    Files are found recursively in the directory matching the symbol.
    """
    file_pattern = f"*{symbol}*.txt"
    return list(Path(path).rglob(file_pattern))


def print_statistics(label, spans, lens, vols):
    """
    Helper function to print min, max, and mean for the given statistics (spans, lens, vols).
    """
    print(
        f"{label} spans: min={np.min(spans):.2f}, max={np.max(spans):.2f}, mean={np.mean(spans):.2f}"
    )
    print(f"{label} lens: min={np.min(lens):.2f}, max={np.max(lens):.2f}, mean={np.mean(lens):.2f}")
    print(f"{label} vols: min={np.min(vols):.2f}, max={np.max(vols):.2f}, mean={np.mean(vols):.2f}")


def calculate_depth_statistics(paths):
    """
    Compute and print statistics (spans, lens, volumes) for bid and ask data in the provided files.
    """
    ...
    for path in paths:
        with open(path, "r") as f:
            for line in f:
                ...
                bids = [float(bid[0]) for bid in entry.get("bids")]
                asks = [float(ask[0]) for ask in entry.get("asks")]

                bid_spans.append(np.ptp(bids))  # Peak-to-peak (max-min)
                ask_spans.append(np.ptp(asks))
                bid_lens.append(len(bids))
                ask_lens.append(len(asks))
                bid_vols.append(np.sum([float(bid[1]) for bid in entry.get("bids")]))
                ask_vols.append(np.sum([float(ask[1]) for ask in entry.get("asks")]))

    # Print statistics
    print_statistics("Bid", bid_spans, bid_lens, bid_vols)
    print_statistics("Ask", ask_spans, ask_lens, ask_vols)
    print(f"Bad lines: {bad_lines}")


def process_files_to_features(paths):
    """
    Process depth data files and generate CSV files with extracted features.
    """
    ...
    for path in paths:
        ...
        with open(path, "r") as f:
            for line in f:
                ...
                # Skip non 1-minute data
                timestamp = entry.get("timestamp")
                if timestamp % 60_000 != 0:
                    continue

                # Convert price-volume strings to floats
                entry["bids"] = [[float(x[0]), float(x[1])] for x in entry.get("bids")]
                entry["asks"] = [[float(x[0]), float(x[1])] for x in entry.get("asks")]
                table.append(entry)

        # Transform the table into a DataFrame and compute features
        df = depth_to_df(table)
        df = df.reset_index().rename(columns={"index": "timestamp"})

        # Adjust timestamp to match 1-minute intervals
        df["timestamp"] = df["timestamp"].shift(periods=1)

        # Save processed data to CSV
        output_file = path.with_suffix(".csv").name
        df.to_csv(output_file, index=False, float_format="%.4f")

        print(f"Processed {path} with {bad_lines} bad lines.")
...
#     # Uncomment to calculate depth statistics
#     # calculate_depth_statistics(paths)

#     # Uncomment to process files into feature CSVs
#     # process_files_to_features(paths)
