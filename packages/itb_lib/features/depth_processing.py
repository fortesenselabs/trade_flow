...
def discretize(side: str, depth: list, bin_size: float, start: float):
    ...
    for b in range(bin_count):
        ...
        if first_point_id is None:  # Bin is empty
            ...
            bin_volume += (
                prev_volume * price_coeff
            )  # Each point in the bin contributes to this bin final value
...
def discretize_ask(depth: list, bin_size: float, start: float):
    ...
    for i, point in enumerate(depth):
        if (
            point[0] <= bin_start
        ):  # Point belongs to previous bin (when start is in the middle of series)
            prev_point = point
            continue
