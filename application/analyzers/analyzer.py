...
class Analyzer(BaseAnalyzer):
    ...
    def store_klines(self, data: dict):
        ...
        for symbol, klines in data.items():
            ...
            if existing_indexes:  # If there is overlap with new klines
                ...
                del klines_data[
                    start:
                ]  # Delete starting from the first kline in new data (which will be added below)
                if (
                    len(klines) < num_deleted
                ):  # It is expected that we add same or more klines than deleted
                    log.error(
                        "More klines is deleted by new klines added, than we actually add. Something woring with timestamps and storage logic."
                    )
            ...
            for i, kline in enumerate(self.klines.get(symbol)):
                ...
                if i > 0:
                    if ts - prev_ts != interval_length_ms:
                        log.error(
                            "Wrong sequence of klines. They are expected to be a regular time series with 1m frequency."
                        )
                prev_ts = kline[0]

            # Debug message about the last received kline end and current ts (which must be less than 1m - rather small delay)
            log.debug(
                f"Stored klines. Total {len(klines_data)} in db. Last kline end: {self.get_last_kline_ts(symbol)+interval_length_ms}. Current time: {now_ts}"
            )
    ...
    def analyze(self, ignore_last_rows=False):
        ...
        if not data_sources:
            data_sources = [
                {"folder": App.config["symbol"], "file": "klines", "column_prefix": ""}
            ]
            ...
            if ds.get("file") == "klines":
                try:
                    ...
                    if df.isnull().any().any():
                        null_columns = {
                            k: v for k, v in df.isnull().any().to_dict().items() if v
                        }
                        log.warning(
                            f"Null in source data found. Columns with Null: {null_columns}"
                        )
                    ...
                    log.error(
                        f"Error in klines_to_df method: {e}. Length klines: {len(klines)}"
                    )
                    return
            else:
                log.error(
                    "Unknown data sources. Currently only 'klines' is supported. Check 'data_sources' in config, key 'file'"
                )
                return
            ds["df"] = df
        ...
        for fs in feature_sets:
            df, feats = generate_feature_set(
                df, fs, last_rows=last_rows if not ignore_last_rows else 0
            )
        ...
        if predict_df.isnull().any().any():
            null_columns = {
                k: v for k, v in predict_df.isnull().any().to_dict().items() if v
            }
        ...
        for fs in train_feature_sets:
            fs_df, feats, _ = predict_feature_set(
                predict_df, fs, App.config, self.models
            )
        ...
        for r in range(2, check_row_count):
            ...
            if not np.all(comp_idx):
                log.warning(
                    f"Newly computed row is not equal to the previously computed row for '{idx}'. NEW: {new_row[~comp_idx].to_dict()}. OLD: {old_row[~comp_idx].to_dict()}"
                )
