...
def generate_features_tsfresh(df, config: dict, last_rows: int = 0):
    ...
    if not column_names:
        raise ValueError(
            f"No input column for feature generator 'stats': {column_names}"
        )
...
def generate_features_talib(df, config: dict, last_rows: int = 0):
    ...
    * Post-processing and pre-processing parameters:
    ...
    columns = {
        arg: df[col_name].interpolate() for arg, col_name in column_names.items()
    }
    ...
    for func_name in func_names:
        ...
        is_streamable_function = (
            fn.function_flags is None
            or "Function has an unstable period" not in fn.function_flags
        )
...
def _convert_to_relative(fn_outs: list, rel_base, rel_func, percentage):
    ...
    for i, feature in enumerate(fn_outs):
        if not rel_base:
            ...
            rel_out = fn_outs[
                i
            ]  # No change because it is the last (no next - it is the base)
        elif (rel_base == "prev" or rel_base == "first") and i == 0:
            rel_out = fn_outs[
                i
            ]  # No change because it is the first (no previous - it is the base)
            ...
            if rel_base == "next":
                ...
                raise ValueError(
                    f"Unknown value of the 'rel_base' config parameter: {rel_base=}"
                )
            ...
            if rel_func == "rel":
                ...
                raise ValueError(
                    f"Unknown value of the 'rel_func' config parameter: {rel_func=}"
                )
...
def generate_features_itbstats(df, config: dict, last_rows: int = 0):
    ...
    for func_name in func_names:
        ...
        args = tuple()
        bias = config.get("parameters", {}).get(
            "bias", False
        )  # By default false (as in pandas)
        if func_name.lower() == "scipy_skew":
            ...
            raise ValueError(
                f"Unknown function '{func_name}' of feature generator {'itbstats'}"
            )
...
def lsbm_fn(x):
    ...
    def _get_length_sequences_where(x):
        ...
        if len(x) == 0:
            ...
            res = [
                len(list(group)) for value, group in itertools.groupby(x) if value == 1
            ]
...
def generate_features_itblib(df, config: dict, last_rows: int = 0):
    ...
    if not functions or "close_WMA" in functions:
        ...
        to_drop += add_past_weighted_aggregations(
            df,
            "close",
            weight_column_name,
            np.nanmean,
...
def generate_features_depth(df, use_differences=False):
    ...
    features += add_past_aggregations(
        df, "bids_1", np.nanmean, windows, "", to_drop[-1], 100.0
    )
    ...
    features += add_past_aggregations(
        df, "asks_1", np.nanmean, windows, "", to_drop[-1], 100.0
    )
    ...
    features += add_past_aggregations(
        df, "bids_2", np.nanmean, windows, "", to_drop[-1], 100.0
    )
    ...
    features += add_past_aggregations(
        df, "asks_2", np.nanmean, windows, "", to_drop[-1], 100.0
    )
    ...
    features += add_past_aggregations(
        df, "bids_5", np.nanmean, windows, "", to_drop[-1], 100.0
    )
    ...
    features += add_past_aggregations(
        df, "asks_5", np.nanmean, windows, "", to_drop[-1], 100.0
    )
    ...
    features += add_past_aggregations(
        df, "bids_10", np.nanmean, windows, "", to_drop[-1], 100.0
    )
    ...
    features += add_past_aggregations(
        df, "asks_10", np.nanmean, windows, "", to_drop[-1], 100.0
    )
    ...
    features += add_past_aggregations(
        df, "bids_20", np.nanmean, windows, "", to_drop[-1], 100.0
    )
    ...
    features += add_past_aggregations(
        df, "asks_20", np.nanmean, windows, "", to_drop[-1], 100.0
    )
    ...
    for i, threshold in enumerate(thresholds):
        ...
        if threshold > 0.0:  # Max high
            if abs(threshold) >= 0.75:  # Large threshold
                ...
                df[out_name] = (
                    df[column_name] <= threshold
                )  # All highs are less than the threshold
        else:  # Min low
